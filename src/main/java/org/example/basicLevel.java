...
public class basicLevel extends JPanel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (bonusTimer > 0) {
            bonusTimer -= DELAY;
        } else if (bonusVisible) {
            bonusVisible = false;  // Hide the bonus fruit after time expires
        }
    ...
    private void restartGame() {
        ...
        inGame = true;
        locateApple();
        bonusVisible = false;  // Hide the bonus fruit
        bonusTimer = 0;
        timer.start();
        restartButton.setVisible(false);
        quitButton.setVisible(false);
        advice = getRandomAdvice();  // Get new advice after restart
        repaint();
    }

    public static void main(String[] args) {
        JFrame frame = new JFrame("Snake Game");
        basicLevel game = new basicLevel();
        frame.add(game);
        frame.pack();
